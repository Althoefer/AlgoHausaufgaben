/**
 * Class BinTree implements a binary search tree
 * 
...
public class BinTree {
	...
	/**
	 * Constructor BinTree creates an empty binary search tree
	 */
	public BinTree() {
		this.root = null;
	}
	...
	/**
	 * Searches the node with the value x. If the node does not exist, null is
	 * returned.
	 * 
	 * @param x
	 *            the value the node we search has
	...
	/**
	 * Searches for the node with the value x, then returns its parent. If the
	 * node does not exist, or is the root, null is returned.
	 * 
	 * @param x
	 *            the value the ndoe we search has
	 ...
	 * 
	 * @param x
	 *            value the node we search has
	 * @return True if the node does exist False if the node does not exist
	 */
	 ...
	 *             if a node with value x already exists
	 */
	public void insert(int x) throws ArithmeticException {
		if (root == null) {
	 ...
	 * 
	 * @param x
	 *            value of the node that should be removed
	 * @throws ArithmeticException
	 *             if a node with that value does not exist
	 */
	public void remove(int x) throws ArithmeticException {
	...
	private void remove(int x, TreeNode n) {
		// now we have to figure out, if the node is a leaf,
		// has 1 child or 2 children
		if (n.left == null && n.right == null) { // node is a leaf
			...
			while (temp.left != null) { // go to the node the furthest on the
										// left in the right tree
				temp = temp.left;
			}
